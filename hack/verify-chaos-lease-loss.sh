...
def validate(root):
    ...
    acquired=one(initial,"event","acquire_succeeded"); ready=one(initial,"event","ready_state_changed"); lost=one(initial,"event","keepalive_lost")
    ...
    for row in (acquired,ready,lost,shutdown,reacquired,reready):
        if row.get("worker_id")!=summary.get("worker_id") or row.get("instance_id")!=summary.get("instance_id"): reject("raw identity mismatch")
    ...
    if not initial_lease or not recovery_lease or initial_lease==recovery_lease: reject("recovery must acquire a different lease")
    ...
    order="initial_identity_acquired initial_ready pre_probe_finished allocator_started fault_injected identity_lease_lost shutdown_triggered authority_barrier_completed restore_started etcd_healthy identity_released recovery_started recovery_identity_acquired recovery_ready allocator_finished post_probe_finished smoke_finished".split()
    order=[f"{key}_at_unix_ns" for key in order]
    ...
    if not timeline or pre.get("probe_timeline_key")!=timeline or post.get("probe_timeline_key")!=timeline: reject("timeline identity changed")
    ...
    for row in logical:
        ...
        retries+=len(attempts)-1; prior_attempt=None
        for index,attempt in enumerate(attempts,1):
            ...
            failed=connect=="failed" or rpc=="grpc_error"
            if failed: failed_attempts+=1
        refreshes=row.get("route_refreshes")
        if not isinstance(refreshes,list) or (refreshes and not any(attempt.get("rpc")=="grpc_error" for attempt in attempts)): reject("refresh without failed RPC")
...
def self_test():
    ...
    write("degrade-observation.txt",{"process_exit_observed_at_unix_ns":ns(605_000_000),"process_exit_status":0,"identity_lease_lost_at_unix_ns":ns(500_000_000),"shutdown_triggered_at_unix_ns":ns(600_000_000)})
    ...
    trace[2]["attempts"][0]["connect"]="connected"
    ...
    def forbidden(root):
        ...
        item["started"]=[ns(810_000_000),810_000_000]; item["finished"]=[ns(1_000_000_000),1_000_000_000]; item["attempts"][0]["started"]=[ns(820_000_000),820_000_000]; item["attempts"][0]["finished"]=[ns(850_000_000),850_000_000]; item["response_received_unix_ns"]=ns(850_000_000); save(root,"fault-span-trace.jsonl",values)
    def clock_jump(root):
        values=load_jsonl(root/"fault-span-trace.jsonl"); item=values[4]
    ...
    def delayed_failure(root):
        values=load_jsonl(root/"fault-span-trace.jsonl"); item=values[3]
        item["started"]=[ns(460_000_000),460_000_000]; item["attempts"][0]["started"]=[ns(470_000_000),470_000_000]; item["attempts"][0]["finished"]=[ns(490_000_000),490_000_000]; save(root,"fault-span-trace.jsonl",values)
    def first_failure(root):
        values=load_jsonl(root/"fault-span-trace.jsonl"); item=values[2]; inside=json.loads(json.dumps(values[3]))
        item.update({"logical_outcome":"failure","response_received_unix_ns":0}); item["attempts"][0]["rpc"]="grpc_error"; item.pop("range_start"); item.pop("range_end")
        values[3]=logical(2,460_000_000,490_000_000,"success",100); inside["ordinal"]=3; values.insert(4,inside); values[5]["ordinal"]=4
        values[-1].update({"logical_record_count":4,"logical_success_count":2,"logical_failure_count":2,"failed_allocation_attempt_count":2}); save(root,"fault-span-trace.jsonl",values)
    ...
    def success_before_later(root):
        values=load_jsonl(root/"fault-span-trace.jsonl"); item=values[2]
        item["attempts"].append({"attempt":2,"connect":"reused","rpc":"grpc_error","started":[ns(445_000_000),445_000_000],"finished":[ns(448_000_000),448_000_000]}); save(root,"fault-span-trace.jsonl",values)
    mutations=[
        ("raw-time",lambda root:kv(root,"summary.txt","identity_lease_lost_at_unix_ns",1)), ("reused-lease",lambda root:row(root,0,"lease_id",11,"recovery-chronos.log")),
        ("missing-loss",lambda root:drop_row(root,2,"initial-chronos.log")), ("missing-shutdown",lambda root:drop_row(root,3,"initial-chronos.log")),
        ("bad-order",lambda root:kv(root,"summary.txt","restore_started_at_unix_ns",1)), ("missing-process-observer",lambda root:kv(root,"degrade-observation.txt","process_exit_observed_at_unix_ns",0)),
        ("timeline",lambda root:kv(root,"pre-fault-probe.log","probe_timeline_key","other")), ("setup",lambda root:row(root,0,"outcome","failure")),
        ("allocator-start-binding",lambda root:kv(root,"summary.txt","allocator_started_at_unix_ns",ns(400_000_000))), ("allocator-finish-binding",lambda root:row(root,-1,"finished",[ns(1_400_000_000),1_400_000_000])),
        ("terminal",lambda root:row(root,-1,"trace_limit_exhausted",True)), ("attempt-count",lambda root:row(root,-1,"failed_allocation_attempt_count",0)),
        ("logical-count",lambda root:row(root,-1,"logical_success_count",0)), ("refresh-accounting",lambda root:row(root,3,"route_refreshes",[{"outcome":"bogus"}])),
        ("connect-rpc-contradiction",lambda root:attempt(root,2,"connect","failed")), ("unknown-connect",lambda root:attempt(root,2,"connect","mystery")),
        ("failure-ending-success",lambda root:attempt(root,3,"rpc","success")), ("attempt-clock-drift",lambda root:attempt(root,3,"started",[ns(650_000_000),530_000_000])),
        ("success-before-later",success_before_later), ("ordinal",lambda root:row(root,3,"ordinal",9)),
        ("forbidden-success",forbidden), ("delayed-failure",delayed_failure),
        ("first-success-range",first_failure), ("between-record-clock-jump",clock_jump),
        ("bench-count",lambda root:kv(root,"fault-span-bench.log","allocate_requests_total",4)), ("smoke",lambda root:kv(root,"recovery-bench.log","allocation_failed_total",1)),
        ("smoke-throughput",lambda root:kv(root,"recovery-bench.log","req_per_sec",9)),
        ("smoke-latency",lambda root:kv(root,"recovery-bench.log","latency_p95_us",500001))]
    expected={"forbidden-success":"success response inside forbidden interval","between-record-clock-jump":"between-record clock jump","delayed-failure":"no logical failure inside forbidden interval","first-success-range":"pre/trace/post ranges are not monotonic"}
    try:
        validate(base)
