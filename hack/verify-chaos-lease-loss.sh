...
def self_test():
    ...
    initial=[{"timestamp":rfc(0),"event":"acquire_succeeded","lease_id":11,**identity},{"timestamp":rfc(100_000_000),"event":"ready_state_changed",**identity},{"timestamp":rfc(500_000_000),"event":"keepalive_lost",**identity},{"timestamp":rfc(600_000_000),"shutdown_trigger":"identity_lease_lost",**identity},{"timestamp":rfc(0),"event":"preflight_passed","build_commit":"abc",**identity}]
    ...
    mutations=[
        ("raw-time",lambda root:kv(root,"summary.txt","identity_lease_lost_at_unix_ns",1)), ("reused-lease",lambda root:row(root,0,"lease_id",11,"recovery-chronos.log")), ("missing-loss",lambda root:drop_row(root,2,"initial-chronos.log")), ("missing-shutdown",lambda root:drop_row(root,3,"initial-chronos.log")),
