...
BENCH_ALLOCATE_REQUEST_TIMEOUT_MS="${CHRONOS_REBALANCE_ALLOCATE_REQUEST_TIMEOUT_MS:-10000}"
CONTROL_BENCH_IDEMPOTENCY="${CHRONOS_REBALANCE_CONTROL_BENCH_IDEMPOTENCY:-false}"
...
TRANSFER_TARGET_GENERATORS="${CHRONOS_REBALANCE_TARGET_GENERATORS:-}"
...
SAFETY_GAP_MS="${CHRONOS_REBALANCE_SAFETY_GAP_MS:-500}"
ALLOCATE_SUCCESS_PER_SEC_MIN="${CHRONOS_REBALANCE_ALLOCATE_SUCCESS_PER_SEC_MIN:-5}"
ALLOCATE_FAILED_TOTAL_MAX="${CHRONOS_REBALANCE_ALLOCATE_FAILED_TOTAL_MAX:-0}"
ALLOCATE_LATENCY_P95_US_MAX="${CHRONOS_REBALANCE_ALLOCATE_LATENCY_P95_US_MAX:-500000}"
ALLOCATE_LATENCY_P999_US_MAX="${CHRONOS_REBALANCE_ALLOCATE_LATENCY_P999_US_MAX:-1000000}"
ROUTE_REFRESH_P95_US_MAX="${CHRONOS_REBALANCE_ROUTE_REFRESH_P95_US_MAX:-500000}"
ROUTE_REFRESH_P999_US_MAX="${CHRONOS_REBALANCE_ROUTE_REFRESH_P999_US_MAX:-1000000}"
TRANSFER_LATENCY_P95_US_MAX="${CHRONOS_REBALANCE_TRANSFER_LATENCY_P95_US_MAX:-5000000}"
TRANSFER_LATENCY_P999_US_MAX="${CHRONOS_REBALANCE_TRANSFER_LATENCY_P999_US_MAX:-1000000}"
...
bench_allocate_request_timeout_ms=${BENCH_ALLOCATE_REQUEST_TIMEOUT_MS}
control_bench_idempotency=${CONTROL_BENCH_IDEMPOTENCY}
...
ownership_plan_id=${TIMELINE_NAMESPACE}
generator_ownership_modulo=2
generator_ownership_remainder_a=0
generator_ownership_remainder_b=1
safety_gap_ms=${SAFETY_GAP_MS}
allocate_success_per_sec_min=${ALLOCATE_SUCCESS_PER_SEC_MIN}
allocate_failed_total_max=${ALLOCATE_FAILED_TOTAL_MAX}
...
transfer_latency_p999_us_max=${TRANSFER_LATENCY_P999_US_MAX}
...
env \
  ...
  CHRONOS_OWNERSHIP_PLAN_ID="${TIMELINE_NAMESPACE}" \
  CHRONOS_GENERATOR_OWNERSHIP_MODULO=2 \
  CHRONOS_GENERATOR_OWNERSHIP_REMAINDER=0 \
...
env \
  ...
  CHRONOS_OWNERSHIP_PLAN_ID="${TIMELINE_NAMESPACE}" \
  CHRONOS_GENERATOR_OWNERSHIP_MODULO=2 \
  CHRONOS_GENERATOR_OWNERSHIP_REMAINDER=1 \
  CHRONOS_SAFETY_GAP_MS="${SAFETY_GAP_MS}" \
...
env \
  ...
  CHRONOS_CONTROL_BENCH_ALLOCATE_REQUEST_TIMEOUT_MS="${BENCH_ALLOCATE_REQUEST_TIMEOUT_MS}" \
  CHRONOS_CONTROL_BENCH_IDEMPOTENCY="${CONTROL_BENCH_IDEMPOTENCY}" \
  ...
  CHRONOS_CONTROL_BENCH_TRANSFER_TARGET_OWNER_ENDPOINTS="${ADVERTISE_ENDPOINT_A},${ADVERTISE_ENDPOINT_B}" \
  CHRONOS_CONTROL_BENCH_ROUTE_TO_OWNERS=true \
...
assert_metric_at_most "allocate_failed_total" "${REBALANCE_LOG}" "${ALLOCATE_FAILED_TOTAL_MAX}"
assert_metric_at_most "allocate_latency_p95_us" "${REBALANCE_LOG}" "${ALLOCATE_LATENCY_P95_US_MAX}"
assert_metric_at_most "allocate_latency_p999_us" "${REBALANCE_LOG}" "${ALLOCATE_LATENCY_P999_US_MAX}"
assert_metric_at_most "route_refresh_p95_us" "${REBALANCE_LOG}" "${ROUTE_REFRESH_P95_US_MAX}"
assert_metric_at_most "route_refresh_p999_us" "${REBALANCE_LOG}" "${ROUTE_REFRESH_P999_US_MAX}"
assert_metric_at_most "transfer_latency_p95_us" "${REBALANCE_LOG}" "${TRANSFER_LATENCY_P95_US_MAX}"
assert_metric_at_most "transfer_latency_p999_us" "${REBALANCE_LOG}" "${TRANSFER_LATENCY_P999_US_MAX}"
