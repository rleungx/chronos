...
lazy_static! {
    pub static ref TSO_ALLOCATE_TOTAL: IntCounter = register_int_counter_metric(
        "tso_allocate_total",
        "Total number of TSO allocation requests"
    );
    pub static ref TSO_ALLOCATE_COUNT_TOTAL: IntCounter = register_int_counter_metric(
        "tso_allocate_count_total",
        "Total number of timestamps allocated"
    );
    pub static ref TSO_ALLOCATE_LATENCY: Histogram = register_histogram_metric(
        "tso_allocate_latency_seconds",
        "Latency of TSO allocation in seconds"
    );
    pub static ref TSO_SEQUENCE_UTILIZATION: IntGauge = register_int_gauge_metric(
        "tso_sequence_utilization",
        "Sequence number utilization within the current millisecond"
    );
    pub static ref TSO_LEASE_EXPIRED_TOTAL: IntCounter = register_int_counter_metric(
        "tso_lease_expired_total",
        "Total number of lease expiration events"
    );
    pub static ref TSO_CLOCK_BACKWARDS_TOTAL: IntCounter = register_int_counter_metric(
        "tso_clock_backwards_total",
        "Total number of clock backwards events"
    );
    pub static ref TSO_METADATA_LATENCY: HistogramVec = register_histogram_vec_metric(
        "tso_metadata_latency_seconds",
        "Metadata store request latency in seconds",
        &["op"]
    );
    pub static ref TSO_METADATA_ERRORS_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_metadata_errors_total",
        "Metadata store request errors",
        &["op"]
    );
    pub static ref TSO_MAINTENANCE_TRACKED_TIMELINES: IntGauge = register_int_gauge_metric(
        "tso_maintenance_tracked_timelines",
        "Number of timelines tracked by the maintenance scheduler"
    );
    pub static ref TSO_MAINTENANCE_EXTEND_TOTAL: IntCounter = register_int_counter_metric(
        "tso_maintenance_extend_total",
        "Total number of authority window extension attempts triggered by maintenance"
    );
    pub static ref TSO_TIMELINE_PROXY_LANES: IntGauge = register_int_gauge_metric(
        "tso_timeline_proxy_lanes",
        "Current number of timeline proxy lanes"
    );
    pub static ref TSO_TIMELINE_PROXY_WAIT: Histogram = register_histogram_metric(
        "tso_timeline_proxy_wait_seconds",
        "Time spent waiting on timeline proxy serialization"
    );
    pub static ref TSO_TIMELINE_PROXY_SATURATED_TOTAL: IntCounter = register_int_counter_metric(
        "tso_timeline_proxy_saturated_total",
        "Total number of timeline proxy saturation events"
    );
    pub static ref TSO_TIMELINE_PROXY_TIMEOUT_TOTAL: IntCounter = register_int_counter_metric(
        "tso_timeline_proxy_timeout_total",
        "Total number of timeline proxy timeout events"
    );
    pub static ref TSO_WATCH_KEEPALIVE_DROPPED_TOTAL: IntCounter = register_int_counter_metric(
        "tso_watch_keepalive_dropped_total",
        "Total number of watch keepalive events dropped due to backpressure"
    );
    pub static ref TSO_WATCH_ROUTE_SEND_TIMEOUT_TOTAL: IntCounter = register_int_counter_metric(
        "tso_watch_route_send_timeout_total",
        "Total number of watch route/event sends that timed out due to slow consumers"
    );
    pub static ref TSO_WATCH_RESYNC_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_watch_resync_total",
        "Route watch resync lifecycle events",
        &["event"]
    );
    pub static ref TSO_TIMELINE_RUNTIME_CACHE_ENTRIES: IntGauge = register_int_gauge_metric(
    ...
    pub static ref TSO_STARTUP_PREFLIGHT_FAIL_TOTAL: IntCounterVec =
        register_int_counter_vec_metric(
            "tso_startup_preflight_fail_total",
            "Total number of startup preflight failures",
            &["stage"]
        );
    pub static ref TSO_STARTUP_BOOTSTRAP_FAIL_TOTAL: IntCounterVec =
        register_int_counter_vec_metric(
            "tso_startup_bootstrap_fail_total",
            "Total number of startup bootstrap failures",
            &["stage"]
        );
    pub static ref TSO_STARTUP_READY: IntGauge = register_int_gauge_metric(
        "tso_startup_ready",
        "Whether the service has completed startup and is ready"
    );
    pub static ref TSO_IDENTITY_LEASE_EVENTS_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_identity_lease_events_total",
        "Identity lease lifecycle events",
        &["event"]
    );
    pub static ref TSO_SHUTDOWN_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_shutdown_total",
        "Controlled shutdown events",
        &["reason"]
    );
    pub static ref TSO_WORKER_READINESS_TRANSITIONS_TOTAL: IntCounterVec =
        register_int_counter_vec_metric(
            "tso_worker_readiness_transitions_total",
            "Worker readiness transitions by state and reason",
            &["from_state", "to_state", "reason"]
        );
    pub static ref TSO_TRANSFER_OUTCOMES_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_transfer_outcomes_total",
        "Transfer and failover action outcomes",
        &["action", "reason", "outcome"]
    );
    pub static ref TSO_RECOVERY_EVENTS_TOTAL: IntCounterVec = register_int_counter_vec_metric(
        "tso_recovery_events_total",
        "Recover-and-continue events by component, operation, and reason",
        &["component", "operation", "reason"]
    );
    pub static ref TSO_BUILD_INFO: IntGaugeVec = register_int_gauge_vec_metric(
        "tso_build_info",
        "Chronos build identity labeled by version and commit",
        &["version", "commit"]
    );
    pub static ref TSO_MIXED_VERSION_CONTRACT_INFO: IntGaugeVec = register_int_gauge_vec_metric(
        "tso_mixed_version_contract_info",
        "Chronos mixed-version contract identity",
        &["contract_id"]
    );
    pub static ref TSO_VERSION_COMPATIBILITY_MISMATCH_TOTAL: IntCounterVec =
        register_int_counter_vec_metric(
