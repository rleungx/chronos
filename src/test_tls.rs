use crate::{TsoConfig, TsoSecurityMode};
use std::sync::OnceLock;

#[cfg(unix)]
use std::os::unix::fs::PermissionsExt;

pub fn readable_test_tls_paths() -> (&'static str, &'static str, &'static str) {
    static PATHS: OnceLock<(String, String, String)> = OnceLock::new();
    let (cert, key, ca) = PATHS.get_or_init(|| {
        let dir = std::env::temp_dir().join("chronos-shared-test-tls");
        std::fs::create_dir_all(&dir).unwrap();
        let cert = dir.join("server.crt");
        let key = dir.join("server.key");
        let ca = dir.join("ca.pem");
        std::fs::write(&cert, b"shared-test-cert").unwrap();
        std::fs::write(&key, b"shared-test-key").unwrap();
        std::fs::write(&ca, b"shared-test-ca").unwrap();
        #[cfg(unix)]
